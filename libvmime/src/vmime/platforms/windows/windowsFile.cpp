...
void windowsFileSystemFactory::reportError(const vmime::utility::path& path, const int err)
{
	...
	throw vmime::exceptions::filesystem_exception(charset::WstringToUtf8(desc), path);
}
...
: m_path(path)
{
    string s_Utf8 = windowsFileSystemFactory::pathToStringImpl(path);
    m_nativePath = charset::Utf8ToWstring(s_Utf8);
}
...
{
	// Fix by Elmue: Use Widechar API
	DWORD dwFileAttribute = GetFileAttributesW(m_nativePath.c_str());
...
bool windowsFile::exists() const
{
	...
	HANDLE hFind = FindFirstFileW(m_nativePath.c_str(), &findData);
...
{
	// Fix by Elmue: Use Widechar API
    string s_Utf8 = windowsFileSystemFactory::pathToStringImpl(newName);
    const vmime::wstring newNativeName = charset::Utf8ToWstring(s_Utf8);

	if (MoveFileW(m_nativePath.c_str(), newNativeName.c_str()))
	{
		m_path = newName;
...
{
	// Fix by Elmue: Use Widechar API
	if (!DeleteFileW(m_nativePath.c_str()))
		windowsFileSystemFactory::reportError(m_path, GetLastError());
}
...
void windowsFile::createDirectoryImpl(const vmime::utility::file::path& fullPath, const vmime::utility::file::path& path, const bool recursive)
{
	// Fix by Elmue: Use Widechar API
    string s_Utf8 = windowsFileSystemFactory::pathToStringImpl(path);
    const vmime::wstring nativePath = charset::Utf8ToWstring(s_Utf8);
	...
	// Fix by Elmue: Use Widechar API
	if (!CreateDirectoryW(nativePath.c_str(), NULL))
		windowsFileSystemFactory::reportError(fullPath, GetLastError());
}

// Fix by Elmue: Use Widechar API
windowsFileIterator::windowsFileIterator(const vmime::utility::file::path& path, const vmime::wstring& nativePath)
...
{
	// Fix by Elmue: Use Widechar API
    string s_Utf8 = charset::WstringToUtf8(m_findData.cFileName);

	ref <vmime::utility::file> pFile = vmime::create <windowsFile>
		(m_path / vmime::utility::file::path::component(s_Utf8));
...
{
	// Fix by Elmue: Use Widechar API
	m_hFind = FindFirstFileW(m_nativePath.c_str(), &m_findData);
	if (m_hFind == INVALID_HANDLE_VALUE)
	{
...
{
	// Fix by Elmue: Use Widechar API
	vmime::wstring s(m_findData.cFileName);
	return (s == L"." || s == L"..");
}

windowsFileReader::windowsFileReader(const vmime::utility::file::path& path, const vmime::wstring& nativePath)
...
// Fix by Elmue: Use Widechar API
windowsFileWriter::windowsFileWriter(const vmime::utility::file::path& path, const vmime::wstring& nativePath)
: m_path(path), m_nativePath(nativePath)
{
}
