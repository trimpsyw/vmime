...
public: 
	...
	string	GetFrom();
    void    GetTo(vector<string>& pi_Emails);
    void    GetCc(vector<string>& pi_Emails);
	string	GetSubject();
    int		GetDate();
	string	GetBody(bool plaintext = true);
	size_t	GetAttachmentCount();
    bool    GetAttachmentAt(size_t u32_Index, string& s_Name, string& s_ContentType, string& s_Data);
	size_t	GetEmbeddedObjectCount();
    bool    GetEmbeddedObjectAt(size_t u32_Index, string& s_Id, string& s_ContentType, string& s_Data);
